...
func unwrapExpectedError(err error) error {
	var errInvalidReferenceFormat InvalidReferenceFormatError
	...
	if errors.As(err, &errReferenceVerification) {
		return errReferenceVerification
	}
...
// according to the reference verification strategy in use for the transaction. It returns the write-ahead log
// entry for the transaction if it was successfully verified.
...
func (mgr *TransactionManager) prepareReferenceTransaction(ctx context.Context, referenceUpdates []*gitalypb.LogEntry_ReferenceUpdate) (_ *updateref.Updater, returnedErr error) {
	...
	defer func() {
		if returnedErr != nil {
			// Clean up the process. We're not interested in the error here as
			// we're returning one already.
			_ = updater.Cancel()
		}
	}()
