...
func TestWriteRef_forceTruncatedRef(t *testing.T) {
	...
	testhelper.RequireGrpcError(t, helper.ErrInternalf(`error when running update-ref command: state update to "commit" failed: EOF, stderr: "fatal: commit: cannot lock ref 'refs/heads/main': unable to resolve reference 'refs/heads/main': reference broken\n"`), err)
}
