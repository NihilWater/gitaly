...
func TestUser_UserCherryPick_expectedOldOID(t *testing.T) {
	...
	_, testRepoCopyPath := gittest.CreateRepository(t, ctx, cfg, gittest.CreateRepositoryConfig{
		Seed: gittest.SeedGitLabTest,
	}) // read-only repo
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedErr: status.Error(codes.Internal, "determine ancestry: exit status 128, stderr: \"fatal: Not a valid object name foobar\\n\""),
		},

		{
			desc: "valid but incorrect ExpectedOldOID",
			getRequest: func() *gitalypb.UserCherryPickRequest {
				destinationBranch := "cherry-picking-dst-2"
				gittest.Exec(t, cfg, "-C", repoPath, "branch", destinationBranch, "master")
			...
			expectedErr: status.Error(codes.Internal, "update reference with hooks: Could not update refs/heads/cherry-picking-dst-2. Please refresh and try again."),
		},
	}

	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			request := tc.getRequest()
