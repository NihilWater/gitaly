...
GitLab defines a very clear and strict workflow for development. The workflow
can be found in this document: [Feature flags in the development of GitLab][rails-feature-development].
As the flag evaluation is invoked in Rails, we have the flexibility of picking the desired
...
At the moment of writing, feature flag definitions and their feature gates are
...
and propagating the evaluation result to Gitaly through gRPC metadata. Other Gitaly's
consumers, such as GitLab Shell or Workhorse, follow the same mechanism. They
propagate the flag evaluation extracted from responses of internal auth APIs.

After Gitaly receives the flags, it continues to pass feature flags between its
internal components via environment variables, or via its gRPC calls.
...
> :warning:️ **This architecture imposes some limitations on how Gitaly uses feature flags**
* All Gitaly feature flags must start with `gitaly_` prefix. Otherwise, they are
...
means Gitaly background jobs, such as repository maintenance, cannot use
feature flags.
...
via chatops. Please mind the `gitaly_` prefix. The rollout process is described
...
GitLab, the company, is one of the biggest organization in GitLab.com. It also
